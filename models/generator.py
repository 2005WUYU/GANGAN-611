...
class GATConvBlock(nn.Module):
    def __init__(self,
        ...
        self.gat = GATConv(
            ...
            out_channels=out_channels // heads,  # 除以heads确保最终维度正确
            heads=heads,
            dropout=dropout,
            concat=True
        )
...
class DecoderBlock(nn.Module):
    def __init__(self,
        ...
        """
        super().__init__()
        self.conv = GATConvBlock(in_channels * 2, out_channels, heads)  # *2因为有跳跃连接
...
class Generator(nn.Module):
    def __init__(self,
        ...
        self.bottleneck = GATConvBlock(
            hidden_channels[-2], hidden_channels[-1], heads
        )
        ...
        self.output_layer = GATConv(
            hidden_channels[0], in_channels, heads=1
        )
    ...
    def decode(self, x, encoded_features):
        ...
        for decoder, (skip_x, skip_edge_index, skip_edge_attr) in zip(
            ...
            curr_x = decoder(
                curr_x, skip_x, skip_edge_index, skip_edge_attr
            )
